...
outputFn = os.path.normpath('output/single_band2/' + name)
if not os.path.exists(os.path.normpath('output/single_band2')):
    os.makedirs('output/single_band2')
...
def train_network(x):
    ...
    with tf.Session() as sess:
        ...
        tempEval = []
        for j in range(int(len(compBand2) / batchSize)+1):
            batch_x = compBand2[j*batchSize:(j+1)*batchSize]
        ...
        tempEval = tempEval[:,1]
        lowThresh = tempEval < 0.05
        tempEval[lowThresh] = tempEval[lowThresh] + 0.05
        highThresh = tempEval > 0.95
        tempEval[highThresh] = tempEval[highThresh] - 0.05
